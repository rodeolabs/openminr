 ...
 * 
 * CRITICAL: Per AGENTS.md section 6.2, NO fallback values allowed.
 * If API key is missing or API fails, this function throws an error.
 */
export async function generateMissionStrategy(goal: string): Promise<MissionStrategy> {
  // Validation: Required API key must be set (no fallbacks per AGENTS.md)
  if (!GROK_API_KEY) {
    throw new Error(
      '[SECURITY VIOLATION] GROK_API_KEY environment variable is required. ' +
      'Hardcoded credentials and fallback generation are strictly prohibited per AGENTS.md. ' +
  ...
  if (!response.ok) {
    throw new Error(`Strategy generation failed: ${response.status} ${response.statusText}`);
  }
