...
<div class="h-full flex flex-col bg-transparent overflow-hidden">
    ...
    <header class="p-4 border-b border-brand-border bg-zinc-950/50 flex flex-col gap-4 shrink-0 backdrop-blur">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">Historical signal analysis and post-incident review.</p>
            </div>
            ...
            <div class="flex gap-2 w-full max-w-md">
                <div class="relative flex-1">
                    ...
                    <input 
                        ...
                        class="w-full bg-black/50 border border-zinc-800 py-2 pl-10 pr-4 text-[10px] font-mono text-white outline-none focus:border-brand-accent/50 transition-all placeholder:text-zinc-700 uppercase"
                    />
                </div>
                <button 
                    onclick={performSearch} 
                    disabled={loading}
                    class="px-4 bg-brand-accent text-black font-black uppercase tracking-wider text-[9px] hover:bg-white transition-colors disabled:opacity-50"
                >
    ...
    <main class="flex-1 overflow-y-auto p-4 custom-scrollbar tactical-grid">
        <div class="max-w-5xl mx-auto space-y-px bg-brand-border">
            {#each incidents as incident (incident.id)}
                <!-- svelte-ignore a11y_click_events_have_key_events -->
                <!-- svelte-ignore a11y_no_static_element_interactions -->
                <div 
                    class="bg-brand-dark/40 backdrop-blur-sm p-4 flex gap-4 hover:bg-zinc-900/80 transition-all group relative border-l-2 border-l-transparent hover:border-l-brand-accent cursor-pointer"
                ...
                >
                    ...
                    <div class="w-28 shrink-0 flex flex-col gap-2">
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center gap-2 text-[9px] font-mono text-zinc-500 uppercase">
                                <Clock size={10} class="text-brand-accent" /> {new Date(incident.created_at).toLocaleDateString()}
                            </div>
                            {#if incident.lat}
                                <div class="flex items-center gap-2 text-[9px] font-mono text-zinc-500 uppercase">
                                    <MapPin size={10} class="text-brand-accent" /> {incident.lat.toFixed(2)}, {incident.lon?.toFixed(2)}
                                </div>
                            {/if}
                        </div>
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-sm font-bold text-zinc-100 group-hover:text-white transition-colors mb-1 uppercase tracking-tight line-clamp-1">
                            {incident.title}
                        </h3>
                        <p class="text-[10px] text-zinc-400 line-clamp-2 leading-relaxed font-medium">
                            {incident.description}
                        </p>
                    </div>

                    <!-- Status Column -->
                    <div class="w-32 shrink-0 flex flex-col items-end justify-between py-1">
                        <span class="px-2 py-0.5 text-[8px] font-black uppercase bg-zinc-900/50 border border-zinc-800 text-zinc-500 rounded-sm tracking-[0.2em]">
                            {incident.status}
                        </span>
                        <div class="flex items-center gap-2 text-[8px] font-black uppercase text-zinc-600 tracking-widest">
                            <Tag size={10} class="text-zinc-700" />
